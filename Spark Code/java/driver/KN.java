...
public class KN {
	...
	public KN(double alpha, double delta, String param, String cov, int n1,Map<Integer, SOAnswer> inGroupContainer) {
		this.alpha = alpha;
		this.delta = delta;
		this.param = param;
		this.cov = cov;
	...
	public void runSystem() {
		...
		if(sysIDs.size()==1){
			...
			double h2 = (n1-1)*(Math.pow(2*alpha/(sysIDs.size()-1), -2.0/(n1-1))-1);
			...
			while(sysIDs.size()>1){
				...
				for(int i=0; i < sysIDs.size();i++){
					...
					for(int j=i+1;j<sysIDs.size();j++){
						...
						int N = (int)(Math.floor((h2*tempS)/(delta*delta)));
						if(t>N){
							...
							if(t*diff<-h2*tempS/(2*delta)+delta*t/2){
								eliCheck=true;
							}
							if(t*diff > h2*tempS/(2*delta)-delta*t/2){
								totalSample = totalSample + t;
