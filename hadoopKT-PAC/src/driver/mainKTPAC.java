	...
	public static class knockoutReducer extends Reducer<IntWritable,Text,IntWritable,Text> {	
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) 
			...
			while(surIDs.size()>1) {
				...
				for(int i = 0; i <numOfComGroups;i++){
					Map<Integer, SOAnswer> inGroupContainer = new HashMap<Integer, SOAnswer>();
					for(int j = 0; j < sysContainer.size();j++) {
						if(j%numOfComGroups == i) {
							inGroupContainer.put(surIDs.get(j), sysContainer.get(surIDs.get(j)));
						}
					}
					
					PPAC ppac = new PPAC(alpha/Math.pow(2, round),1.0/3.0*Math.pow(0.75, round)*delta,param,cov,n1,inGroupContainer);	
					//PPAC ppac = new PPAC(alpha/Math.pow(2, round),delta/startingRound,param,cov,n1,inGroupContainer);	
					
					
					ppac.runSystem();
					_simTime = _simTime+ppac.getSimTime();
					_comTime = _comTime+ppac.getCompareTime();
					stageSampleSize = stageSampleSize + ppac.getTotalSample();
				...
				remain = remain - 0.25*Math.pow(0.75, round)*delta;
				sysContainer.clear();
			...
			//int rinottSampleSize =(int) Math.ceil(sysContainer.get(bestID.get(0)).getFnVar()*Math.pow(rinottH/(delta/startingRound),2))-n1;
			int rinottSampleSize =(int) Math.ceil(sysContainer.get(bestID.get(0)).getFnVar()*Math.pow(rinottH/(remain),2))-n1;
